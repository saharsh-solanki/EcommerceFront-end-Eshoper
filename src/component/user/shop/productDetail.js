...
import { SingleProductApi } from "../../../api/shop/shop";
import { useDispatch } from "react-redux";
import { AddProductToCartApi } from "../../../api/shop/cart";
import { SuccessToast } from "../../toast/base";
export default function ProductDetail() {
  const [productData, setproductData] = React.useState();

  const [productState, setproductState] = React.useState({ quantity: 1 });
  ...
  const HandleInputChange = (event) => {
    var extra_infos = productState.extra_info ? productState.extra_info : {};
    setproductState({
      ...productState,
      extra_info: {
        ...extra_infos,
  ...
  const increaseQyt = () => {
    var add = productState.quantity === 10 ? 0 : 1;
    setproductState({
      ...productState,
      quantity: productState.quantity + add,
    });
    console.log(productState);
  };

  const decreaseQyt = () => {
    var add = productState.quantity === 1 ? 0 : 1;
    setproductState({
      ...productState,
      quantity: productState.quantity - add,
    });
  ...
  React.useEffect(async () => {
    if (productid) {
      var response = await SingleProductApi(productid);
                    ...
                    ? productData.images.images.map((image, index) => {
                        return (
                          ...
                          >
                            <img
                              class="w-100 h-100"
                              src={GetConvertedImage(image.image)}
                              alt="Image"
                            />
                          </div>
                        );
                      })
                    : "Not Image Avaible for this product"}
                </div>
                {productData.images.images ? (
                  <>
                    {" "}
                  ...
                  "Not Image Availble to preview"
                )}
              </div>
            </div>

            <div class="col-lg-7 pb-5">
              <h3 class="font-weight-semi-bold">{productData.product_name}</h3>
              ...
              <div class="d-flex align-items-center mb-4 pt-2">
                ...
                <button
                  onClick={(event) => {
                    if (isAuth) {
                      AddProductToCartApi(productData.id, dispatch, {
                        ...productState,
                      });
                      SuccessToast("Update Cart");
                    } else {
